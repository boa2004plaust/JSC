...
from models.cls_resnet50 import resnet50dfl_cnn
from models.cls_resnet18 import resnet18dfl_cnn
from models.shufflenetv2 import shufflenetv2dfl_cnn
...
parser.add_argument('--arch_cls_t', type=str, default='resnet50', help='classification name')
parser.add_argument('--arch_cls_s', type=str, default='resnet18', help='classification name')
...
sys.stdout = Logger(os.path.join(opt.logdir, opt.name, 'log_train2.txt'))
print(opt)
...
# CLS Networks
print(opt.arch_cls_t)
if opt.arch_cls_t == 'resnet50':
    cls_model_t = resnet50dfl_cnn(num_classes=dataset_num_classes)
    name_t = opt.name.replace('shufflenetv2', 'resnet50')
    state_dict = torch.load(
        'log/log_train_teacher_srnet/'+name_t+'/last_'+name_t+'_base.pth.tar',
        map_location=lambda storage, loc: storage)
    new_state_dict = {}
    for k, v in state_dict.items():
        new_state_dict[k[7:]] = v  #remove `module.`
    cls_model_t.load_state_dict(new_state_dict)
if opt.arch_cls_s == 'resnet18':
    cls_model_s = resnet18dfl_cnn(num_classes=dataset_num_classes)
if opt.arch_cls_s == 'shufflenetv2':
    cls_model_s = shufflenetv2dfl_cnn(num_classes=dataset_num_classes)
...
if opt.cuda:
    ...
    cls_model_t = cls_model_t.cuda()
    cls_model_s = cls_model_s.cuda()
    sr_model = torch.nn.DataParallel(sr_model)
    cls_model_t = torch.nn.DataParallel(cls_model_t)
    cls_model_s = torch.nn.DataParallel(cls_model_s)

# IT Loss
from utils.IT import ITLoss
criterion_kd = ITLoss(512, 512, opt.batchSize, 1.01).cuda()
criterion_C = CrossEntropyLabelSmooth(num_classes=dataset_num_classes, use_gpu=True)

# Optimizers & LR schedulers
cls_optimizer = optim.SGD(cls_model_s.parameters(), lr=opt.lr_cls, momentum=0.9, weight_decay=opt.weight_decay)
...
def test(epoch, test_loader):
    ...
    with torch.no_grad():
        for iter, (imhr, imlr, targets) in enumerate(test_loader):
            ...
            imsr = sr_model(imlr)
            # outputslr = cls_model_s(imsr)
            # outputshr = cls_model_s(imhr)
            outputslr_list = cls_model_s(imsr)
            outputslr = outputslr_list[0] + outputslr_list[1] + 0.1 * outputslr_list[2]
            outputshr_list = cls_model_s(imhr)
            outputshr = outputshr_list[0] + outputshr_list[1] + 0.1 * outputshr_list[2]
...
def train(epoch, train_loader):
    ...
    for iter, (imhr, imlr, train_label) in enumerate(train_loader):
        ...
        imsr = sr_model(imlr)
        # f_s, logit_s = cls_model_s(imsr, is_feat=True)
        f_s_list, logit_s_list = cls_model_s(imsr, is_feat=True)
        f_s = f_s_list[0]
        # print(f_s.shape)
        loss1 = criterion_C( logit_s_list[0], train_label )
        loss2 = criterion_C( logit_s_list[1], train_label )
        loss3 = criterion_C( logit_s_list[2], train_label )
        loss_cls = loss1 + loss2 + 0.1 * loss3
        logit_s = logit_s_list[0] + logit_s_list[1] + 0.1 * logit_s_list[2]
        # loss_cls = criterion_C( logit_s, train_label )
        with torch.no_grad():
            # f_t, logit_t = cls_model_t(imsr, is_feat=True)
            f_t_list, logit_t_list = cls_model_t(imsr, is_feat=True)
            f_t = f_t_list[0]
            # print( f_t.shape )
            # logit_t = logit_t_list[0] + logit_t_list[1] + 0.1 * logit_t_list[2]
            # loss_cls = criterion_C( logit_t, train_label )
            f_t = f_t.detach()


        #### IT Loss
        loss_correlation = criterion_kd.forward_correlation_it(f_s, f_t)
        loss_mutual = criterion_kd.forward_mutual_it(f_s, f_t)
        loss_kd = loss_mutual + 2 * loss_correlation
        ...
        prec1, prec5 = accuracy(logit_s.data, train_label.data, topk=(1, 5))
        top1 += prec1
        top5 += prec5
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    save_checkpoint(cls_model_s.state_dict(),
                    os.path.join(opt.logdir, opt.name),
                    is_best, name=opt.name+'_student')
