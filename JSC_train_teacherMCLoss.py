...
if opt.arch_cls == 'resnet50':
    cls_model = resnet50dfl_cnn(num_classes=dataset_num_classes)
elif opt.arch_cls == 'resnet18':
    cls_model = resnet18dfl_cnn(num_classes=dataset_num_classes)
elif opt.arch_cls == 'shufflenetv2':
    cls_model = shufflenetv2dfl_cnn(num_classes=dataset_num_classes)
else:
...
def train(epoch, train_loader):
    ...
    for iter, (imhr, imlr, train_label) in enumerate(train_loader):
        ...
        imsr = sr_model(imlr)
        outsr_list = cls_model(imsr)
        loss1 = criterion_C(outsr_list[0], train_label)
        loss2 = criterion_C(outsr_list[1], train_label)
        loss3 = criterion_C(outsr_list[2], train_label)
        loss = loss1 + loss2 + 0.1 * loss3
        outsr = outsr_list[0] + outsr_list[1] + 0.1 * outsr_list[2]
